...
def build_graph(checkpointer: Any | None = None) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "LangGraph is required. Run: pip install -e '.[dev,sqlite]' "
    ...
    graph.add_node("fanout_dispatch", fanout_dispatch_node)
    graph.add_node("tool", tool_node)
    graph.add_node("tool_order", tool_order_node)
    graph.add_node("tool_profile", tool_profile_node)
    ...
    graph.add_conditional_edges(
        ...
        {
            "answer": "answer",
            "fanout_dispatch": "fanout_dispatch",
            "clarify": "clarify",
    ...
    graph.add_conditional_edges(
        "fanout_dispatch",
        route_after_fanout_dispatch,
        ["tool_order", "tool_profile"],
    )
    graph.add_edge("tool", "evaluate")
    graph.add_edge("tool_order", "evaluate")
    graph.add_edge("tool_profile", "evaluate")
    graph.add_conditional_edges(
        "evaluate",
        route_after_evaluate,
        {
